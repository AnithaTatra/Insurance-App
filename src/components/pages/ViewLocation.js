...
const UsingFetch = () => {
    ...
    axios.get('http://localhost:4000/api/v3/location/viewLocation')
          ...
          const viewData=(data)=>{
           ...
           console.log("viewUuId...",viewUuId)
            axios.get('http://localhost:4000/api/v3/location/editLocation?uuid='+viewUuId+'')
    
                  .then(response=>{
                      console.log("Response..",response);
                       if(response){
                        history.push('/locationView',{state:response.data.result});
                       }
                  }).catch(error=>{
                      console.log(error.message);
                  })
  ...
  return (
    ...
    <h1>Location List Data</h1>
   <center>
    <tbody>
        <tr >
        <th className="table_border" >S.NO</th>
        <th className="table_borderhide" >UUID</th>
        <th className="table_border">LocationName</th>
        <th className="table_border" >PreviousLocation</th>
        <th className="table_border" >PresentLocation</th>
        ...
        {data.map((item, i) => (
            <tr id={i+1} className="table_border" key={i}>
                ...
                <td className="table_border">{item.locationName}</td>                       
                <td className="table_border">{item.locationPresent}</td>
                <td className="table_border">{item.locationPrevious}</td>
                <td className="table_border">{item.locationStatus}</td>
