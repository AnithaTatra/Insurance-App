    ...
    axios.get('http://localhost:4000/api/v4/lob/viewLob')
          ...
          const viewData=(data)=>{
           ...
           console.log("viewUuId...",viewUuId)
            axios.get('http://localhost:4000/api/v4/lob/editLob?uuid='+viewUuId+'')
    
                  .then(response=>{
                      console.log("Response..",response);
                       if(response){
                        history.push('/LobView',{state:response.data.result});
                       }
                  }).catch(error=>{
                      console.log(error.message);
                  })
  ...
  return (
   ...
   <center>
    <tbody>
        ...
        <th className="table_border">LOBName</th>
        ...
        {data.map((item, i) => (
            <tr id={i+1} className="table_border" key={i}>
                ...
                <td className="table_border">{item.lobName}</td>                       
                <td className="table_border">{item.lobStartDate}</td>
                <td className="table_border">{item.lobEndDate}</td>
                <td className="table_border">{item.lobStatus}</td>
